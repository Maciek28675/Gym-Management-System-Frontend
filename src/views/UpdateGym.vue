<template>
    <form class="max-w-lg mx-auto" @submit.prevent="updateGym">
        <div class="relative z-0 w-full mb-5 group">
            <input type="number" v-model="formData.gym_id" name="floating_first_name" id="floating_first_name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-orange-500 focus:outline-none focus:ring-0 focus:border-orange-600 peer" placeholder=" " required />
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="text" v-model="formData.address" name="floating_password" id="floating_password" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-orange-500 focus:outline-none focus:ring-0 focus:border-orange-600 peer" placeholder=" " />
                ...
                try {
                    ...
                    const updatedFields = Object.fromEntries(
                        Object.entries(this.formData).filter(([key, value]) =>
                            key !== "gym_id" && (typeof value === "string" ? value.trim() !== "" : value !== null && value !== undefined)
                            )
                        );
                    
                    console.log(updatedFields)
                    const response = await axios.put(`http://localhost:5000/api/update_gym/${this.formData.gym_id}`, updatedFields, { headers: authHeader() });
