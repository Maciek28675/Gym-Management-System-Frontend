...
<template>
    <div class="p-6">
      <div class="flex justify-between items-center mb-4">
        <div class="flex items-center space-x-4">
          ...
          <button class="bg-orange-600 text-white font-semibold px-4 py-2 rounded shadow">
            <router-link to="/AddGym">Add Gym</router-link>
          </button>
          <button class="bg-blue-500 text-white px-4 py-2 rounded shadow">  
            <router-link to="/UpdateGym">Modify</router-link>
          </button>
          <button class="bg-red-500 text-white px-4 py-2 rounded shadow">
            <router-link to="/DeleteGym">Delete</router-link>
          </button>
        </div>
      </div>
      <div class="overflow-auto max-h-96" @scroll="onScroll">
        <table class="min-w-full bg-white border rounded-lg shadow">
          ...
          <tbody>
            <tr v-for="gym in filteredGyms" :key="gym.id" class="hover:bg-gray-100">
              <td class="px-4 py-2">{{ gym.gym_id }}</td> 
              <td class="px-4 py-2">{{ gym.name }}</td>
  ...
  export default {
    ...
    methods: {
      async loadGyms() {
        try {
          const response = await axios.get(`http://localhost:5000/api/get_all_gyms`,
            { headers: authHeader(),
              params: {
                limit: this.numberOfgymsToLoad,
                offset: this.offset
              }
            });
            ...
            if(response.status == 200) {
              ...
              this.offset += this.numberOfGymsToLoad;

              if (response.data.length < this.numberOfGymsToLoad) {
                this.hasMore = false;
                this.message = "No more gyms to load";
