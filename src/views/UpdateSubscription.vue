<template>
    <form class="max-w-lg mx-auto" @submit.prevent="updateSubscription">
        <div class="relative z-0 w-full mb-5 group">
            <input type="number" v-model="formData.subscription_id" name="floating_first_name" id="floating_first_name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-orange-500 focus:outline-none focus:ring-0 focus:border-orange-600 peer" placeholder=" " required />
            <label for="floating_first_name" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-orange-600 peer-focus:dark:text-orange-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Subscription ID</label>
        </div>
        <div class="relative z-0 w-full mb-5 group">
            <input type="text" pattern="[A-Za-z]+" v-model="formData.type" name="floating_email" id="floating_email" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-orange-500 focus:outline-none focus:ring-0 focus:border-orange-600 peer" placeholder=" " />
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="number" step="0.01" v-model="formData.price" name="floating_password" id="floating_password" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-orange-500 focus:outline-none focus:ring-0 focus:border-orange-600 peer" placeholder=" " />
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="number" v-model="formData.period" name="repeat_password" id="floating_repeat_password" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-orange-500 focus:outline-none focus:ring-0 focus:border-orange-600 peer" placeholder=" " />
                ...
                try {
                    ...
                    const updatedFields = Object.fromEntries(
                        Object.entries(this.formData).filter(([key, value]) =>
                            key !== "subscription_id" && (typeof value === "string" ? value.trim() !== "" : value !== null && value !== undefined)
                            )
                        );
                    
                    console.log(updatedFields)
                    const response = await axios.put(`http://localhost:5000/api/update_subscription/${this.formData.subscription_id}`, updatedFields, { headers: authHeader() });
